...
void OV2640_Init(void) 
{
	...
	OV2640_DMA_Config(LCD_FB_START_ADDRESS, (160 * 120) / 2);
}
//
...
void OV2640_DMA_Config(uint32_t DMA_Memory0BaseAddr, uint32_t DMA_BufferSize)
{
  ...
  DMA_Handle_dcmi.Init.MemDataAlignment     = DMA_PDATAALIGN_WORD;
...
void OV2640_QQVGAConfig(void)
{
  ...
  {
     if(OV2640_WriteReg(OV2640_QQVGA[i][0], OV2640_QQVGA[i][1]) == HAL_OK)
      ok++;
    else
      fault++;
    HAL_Delay(wait);
...
void OV2640_UXGAConfig(void)
{
  ...
  /* 写入寄存器配置 */
	...
	{
		if(OV2640_WriteReg(OV2640_UXGA[i][0], OV2640_UXGA[i][1]) == HAL_OK)
      ok++;
    else
      fault++;
    HAL_Delay(wait);
